...
Poroelasticity models \js{coupled to discrete fracture flow, neglecting the mechanical response of the fractures, \sout{with discrete fractures}} were studied e.g., in \cite{ganis2014modeling,girault_lubrication,girault2019mixed,bukac2017dimensional,hanowski2018hydromechanical}.
\js{\sout{ where Biot's system in the matrix was coupled with fracture flow based on the cubic law.
In %\cite{hanowski2018hydromechanical}
...
Fracture mechanics, however, plays an important role in the applications. Natural fractures are never smooth, and can be filled by solid material e.g. due to healing \cite{vass}. Hence fracture stiffness, non-penetration and friction conditions have to be taken into account.
Fracture stiffness can be estimated using rock joint models \cite{bandis1983fundamentals}, see also \cite{rutqvist2003role}.
There has been an ongoing research on the numerical analysis and simulation of models involving fracture contact mechanics, see e.g. \cite{garipov2016discrete,franceschini2020algebraically,berge2019finite,blaheta2020bayesian,bonaldi2022numerical}. In these works, the fracture stiffness is not considered.}
\js{Recently,} a general framework for \js{\sout{mixed-dimensional elliptic equations was developed in
%\cite{boon2019stable, boon2017functional}
...
\begin{enumerate}
    ...
    %\cite{showalter2000diffusion,zenisek1984existence}
    ...
    %\cite{mikelic2013convergence}
    , and show the contractivity of the appropriate mapping\js.% with optimal estimates of splitting parameters.
    }}
...
\subsection{DFM model of poroelasticity}

The coupled DFM model is obtained after neglecting the remainders $O(\delta)$ in \eqref{eq:lin_el_frac}, \eqref{eq:interface_el}, \eqref{eq:flow_frac}, \eqref{eq:interface_flow}.
...
\item[(iv)] boundary and \js{initial} conditions:
\begin{align}
...
The model can be extended in many directions, namely to more complex geometries (embedded fractures with variable aperture and their intersections), nonlinear fracture mechanics, % \cite{berge2019finite}
or variable fracture permeability, % \cite{Watanabe2012lower}
