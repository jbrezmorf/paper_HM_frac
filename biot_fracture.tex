...
\thanks{The research was supported %by the project ``RINGEN - research infrastructure upgrade'' No CZ.02.1.01/0.0/0.0/16\_013/0001792, co-funded by the EU Operational Programme ``Research, Development and Education'' and 
by the Euratom research and training programme 2014-2018 under grant agreement No [847593].}}
\author{Jan Březina}
\author{Jan Stebel}
...
In \cite{hanowski2018hydromechanical} the Biot system in matrix is coupled with Darcian fracture flow, but in addition fracture width and length can vary according to rock deformation.
...
The second aim of the paper is the analysis of well-posedness of the mixed-dimensional poroelasticity.
...
The latter approach has a great advantage in the design of numerical schemes where the splitting acts as a preconditioner \cite{white2016block} that allows sequential use of different solvers for flow and mechanics.
...
Finally, in section \ref{sec:comp_test} we present a numerical example that demonstrates the mixed-dimensional model and its iterative solution.
...
Other possible conditions can be taken into account, we refer to \cite{deresiewicz-skalak,gurevich-schoenberg} for more details.
\end{subequations}
...
With help of the compact embedding $H^1(\gamma)\hookrightarrow L^2(\gamma)$ we deduce from \eqref{eq:korn_conv_approx_grad} that $\ep_\tau(\V_{nf})\to \tn 0$ strongly in $L^2(\gamma;\Real^{d\times d})$, $n\to\infty$.
...
In this section we present the numerical solution of a test problem for fluid injection into a fracture.
The computational domain is composed of a rectangle $(0,L)\times(0,H)$ and a fracture $(0,L)\times\{\tfrac H2\}$, $L=25$ m, $H=1$ m.
Initially, the fluid pressure in the whole domain is $p_0=11$ MPa.
Throughout the simulation, the pressure at the right end of the fracture is kept at the initial value $p_0$ while the left end is under an increased pressure $p_{in}=11.9$ MPa.
The boundary of the rectangle is impermeable.
The lower side is mechanically fixed, the left and right sides and fracture ends are subject to the roller boundary condition $\U_{\{m,f\}}\cdot\nn=0$ and the top side is under a constant traction 11 MPa.
...
The problem is solved with help of the simulator Flow123d \cite{flow123d} in the time interval $[0,2000\,\mbox{s}]$.
The computational mesh consists of 6856 triangular elements, the fracture is divided into 500 lines which coincide with sides of adjacent triangular elements.
The flow part of the problem is discretized using the implicit Euler timestepping with the time step $\Delta t=100\,\mbox{s}$ and the diagonalized mixed hybrid finite element method (see \cite{flow123d} for details). The flux, pressure and pressure trace on element sides are approximated using the $RT_0/P_0/P_0^{sides}$ finite elements.
The mechanical part is discretized using $P_1$ finite elements for the displacement.
...
Due to the boundary conditions, the high inlet pressure propagates through the fracture and causes its opening.
For a similar problem, considered in an infinite fracture, an analytical solution has been derived in \cite{wijesinghe}, given by the formula
% 
\eq{ P_f(t,x) = p_0 + (p_{in}-p_0)\erfc\left(\frac12 \sqrt{\frac{\delta}{k_f \mu_f}} \frac{x}{\sqrt{t}}\right). }
% 
The pressure profile in the fracture at selected times is depicted in Figure \ref{fig:test_solution} which shows a good agreement with the analytical prediction.
\begin{figure}[h]
\centering
...
Next we test the choice of the iteration parameters $\beta_m,\beta_f$.
Motivated by Theorem \ref{th:conv_iter}, we set
\eq{ \beta_*:=\tilde\beta\frac{\alpha_*^2}{2(\mu_*+\lambda_*)}, ~*\in\{m,f\}, }
where $\tilde\beta\in[0.3,10]$ is a scaling parameter.
The tolerance for the iterations is set to $tol=10^{-6}$.
In Figure \ref{fig:test_iterations} we show the dependence of number of iterations, necessary to achieve the convergence criterion, on $\tilde\beta$.
...
\end{figure}
It turns out that, in agreement with Theorem \ref{th:conv_iter}, the optimal value of $\tilde\beta$ is close to 1 and its overestimation leads to moderate increase of number of iterations.
On the other hand, for $\tilde\beta<1$ the iterative scheme converges very slowly or diverges.
To verify the optimality of the estimate \eqref{eq:cond_beta}, we performed analogous tests for perturbed conductivity, and elastic moduli.
...
The numerical test confirmed the theoretical convergence result.
