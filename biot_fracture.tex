...
Since the tangential gradient and divergence do not depend on values outside of $\gamma$, they are well-defined even for functions defined only in $\gamma$.
...
\ml{ \label{eq:mixed_dim_problem_el_frac} -\delta\div_\tau\left(\CC_f\avg{\agrad[\uu,\U]}\right) + \delta\alpha_f\nabla_\tau P - \jmp{\ttraction[\uu,\U]}\\
...
We first solve the mechanical and flow subproblem, respectively.
Then we introduce a suitable iterative scheme alternating between the subproblems and prove its convergence.
...
Hence the sequence $\{(p_n,P_n)\}_{n=1}^\infty$ is bounded in $\mathcal X$.
