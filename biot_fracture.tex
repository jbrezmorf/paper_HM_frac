...
\begin{abstract}
The paper deals with derivation and analysis of a poroelasticity model \jb{on} a domain with lower-dimensional fracture.
...
% Many numerical models of fractures rocks use geometries with a lot of intersecting fractures.
% Some of these models consider only flow in fractures (also called DFN --- \textit{discrete fracture networks}) or interaction with the surrounding matrix (DFM --- \textit{discrete fracture-matrix}, in this paper we use the notation \textit{mixed-dimensional} models).
...
be a finite time interval, $\ff$ the body force, $g$ the fluid source and $p_0$ the initial pressure distribution.
...
We recall that the components of the Jacobian matrix and of the tensor divergence are given by $\nabla\vv=\left[\tfrac{\prtl v_i}{\prtl x_j}\right]_{i,j=1}^d$ and $\div\tn A = \left[\sum_{i=1}^d\tfrac{\prtl a_{ij}}{\prtl x_j}\right]_{j=1}^d$, respectively.
...
\ml{ \label{eq:lin_el_frac} -\delta\div_\tau\left(\CC_f\avg{\agrad[\uu,\ubar]}\right) + \delta\alpha_f\avg{\agrad[p,\pbar]} - \jmp{\tt[\uu,\ubar]}\\
...
\ml{ \label{eq:mixed_dim_problem_el_frac} -\delta\div_\tau\left(\CC_f\avg{\agrad[\uu,\U]}\right) + \delta\alpha_f\avg{\agrad[p,P]} - \jmp{\tt[\uu,\U]}\\
...
\end{itemize}
\end{subequations}
We point out that $\U$ and $P$ play the role of the averages $\ubar$ and $\pbar$ in $\Omega_f$.
...
\eq{ \label{eq:mixed_dim_problem_el_frac_perm} \tag{\ref{eq:mixed_dim_problem_el_frac}'} -\delta\div_\tau\left(\CC_f\avg{\agrad[\uu,\U]}\right) + \delta\alpha_f\grad_\tau P - \jmp{\tt[\uu,\U]} = \FF \mbox{ in }\I\times\gamma, }
...
\left.\begin{minipage}{0.85\textwidth}
...
Assuming that $p$ and $P$ are given pressure fields at some time $t\in\I$, we want to solve the following problem:
% find the displacement fields $\uu=\uu(t):\Omega_m\to\Real^d$ and $\U=\U(t):\gamma\to\Real^d$ satisfying the following boundary-value problem:
...
\jb{Indeed, for the diagonal components we have 
\[
  \ep_\tau(\V_\tau) = 0, \quad \V_\nu \otimes \nu = 0,
\]
and thus $\ep_\tau(\V)=0$.}
...
From this we see that $a$ is elliptic with the constant $\alpha = 2C_2\min\{\mu_m,\mu_f\delta\}$.
\end{proof}
...
and
\[
 \norm{(\uu,\U)}_{\Vel} \le \frac{1}{\alpha} \Big( \norm{(\ff,\FF)}_{\Vel^*} + \beta\norm{(p,P)}_\Hf \Big)
\]
where $\beta(\alpha_m, \alpha_f, \delta)$ is a suitable constant.}
\end{theorem}
% 
Now we consider the time-dependent case. 
\jb{
Since the solution mapping $(\ff,\FF), (p,P) \to (\uu, \U)$ is bilinear and bounded, we obtain:
}
\begin{corollary}
...
% where $(\uu(t),\U(t))$ is the solution to \eqref{eq:el_weak} with the loads $(\ff(t),\FF(t))$ and pressures $(p(t),P(t))$, $t\in\I$.
...
We want to 
% find the pressure fields $p:[\I)\times\Omega_m\to\Real$ and $P:[\I)\times\gamma\to\Real$ such that
% \begin{subequations}\label{eq:flow_problem}
% \begin{align}
% S_m\dt p - \div(\tn K_m\nabla p) &= g &&\mbox{ in }\I\times\Omega_m,\\
% \delta S_f\dt P - \delta\div_\tau\left(\tn K_f\avg{\agrad[p,P]}\right) - \jmp{\pphi[p,P]} &= G &&\mbox{ in }\I\times\gamma,\\
% \label{eq:condition_pP} \tn K_m(\nabla p)^\opm\cdot\nnu - \pphi^\opm[p,P] &= 0 &&\mbox{ in }\I\times\gamma,\\
% \label{eq:bc_p} p & = 0 &&\mbox{ on }\I\times\Gamma,\\
% \label{eq:bc_P} P & = 0 &&\mbox{ on }\I\times\prtl\gamma,\\
...
Find $(p,P)\in L^\infty(\I;\Vf)\cap H^1(\I;\Hf)$ such that
\begin{subequations}
    \label{eq:flow_weak}
    \eq{
...
% 
\begin{theorem}
Let $(p_0,P_0)\in\Vf$ and $(g,G)\in L^2(\I;\Hf)$.
...
In particular, $(p,P):=\mathcal F_{\vc\beta}(g,G)$ satisfies the initial conditions 
...
Let $n\in\{0,1,2,...\}$.
Given $(p^n,P^n)\in L^\infty(\I;\Vf)\cap H^1(\I;\Hf)$, we define
...
\jb{
This pair is well defined, in particular 
\[
 \dt\avg{\adiv[\uu^n,\U^n]} = \avg{\adiv[\dt\uu^n,\dt\U^n]} \in L^2(I;L^2(\gamma)).
\]
?? Rather have $\avg{\adiv[\dt\uu^n,\dt\U^n]}$ also above.
}

% 
The above scheme defines the mapping
\eqs{ \mathcal M_{\vc\beta}:(p^n,P^n) \mapsto (p^{n+1},P^{n+1}) }
from $\mathcal X = L^\infty(\I;\Vf)\cap H^1(\I;\Hf)$ into itself.
...
%
\jb{Is it metric? Define as an induced metric of suitable norm.}
%
...
then $\mathcal M_{\vc\beta}$ is a contraction in $L^\infty(\I;\Vf)\cap H^1(\I;\Hf)$ with respect to the distance $\rho_{\vc\beta}$.
...
\jb{Double braces inside the norm??}.
Next we subtract the equations \eqref{eq:flow_problem_beta} 
\jb{? With modified $(g,G)$?}


for $(p^{n+1},P^{n+1})$ and $(p^n,P^n)$, test by $(e_p^{n+1},e_P^{n+1})$, integrate in time and use \eqref{eq:pos_def_K}, which yields for a.a. $t\in\I$:
\ml{ 
    ...
    - \beta_m e_p^n\right)}_{=:e_\sigma^n} e_p^{n+1} - \delta\int_0^t\int_\gamma\avg{\underbrace{\alpha_f\adiv[\ee_\uu^n,\ee_\U^n] - \beta_f e_P^n}_{=:e_\Sigma^n}} e_P^{n\jb{+1}}. 
}
...
\le \frac1{2\omega^2}\left(\rho_{\vc\beta}((p^n,P^n),(p^{n-1},P^{n-1}))\right)^2. }
Hence
\eqs{ \rho_{\vc\beta}((p^{n+1},P^{n+1}),(p^n,P^n)) \le \frac1\omega \rho_{\vc\beta}((p^n,P^n),(p^{n-1},P^{n-1})). }
...
This convergence result can be of its own interest especially for the purpose of numerical solution.
The problem can be further extended to the case of embedded fracture with variable aperture and intersecting fractures, further one should consider contact and friction conditions, to name a few challenging tasks.
Suitable numerical scheme and its analysis will be the subject of a following paper.
...
\section{Auxiliary inequalities and identities}
...
\begin{lemma}
The functions $\tt^\opm$ and $\varphi^\opm$, defined in \eqref{eq:def_t} and \eqref{eq:def_F}, satisfy:
\eq{ \label{eq:ellip_t} \jmp{\tt[\vv,\V]\cdot(\vc w-\vc W)} = \delta\avg{\CC_f\agrad[\vv,\V]:\agrad_\nu[\vc w,\vc W]}, }
...
\mls{ \tt^\opm[\vv,\V]\cdot(\vc w^\opm-\vc W)
...
\ml{ \label{eq:energy_el_mat} \int_{\Omega_m}(\CC_m\nabla\dt\uu:\nabla\dt\uu - \alpha_m\dt p \div\dt\uu) + \int_\gamma\jmp{\tt[\dt\uu,\dt\U]\cdot\dt\uu}\\
...
+ \int_\gamma\alpha_f\jmp{\dt p}\dt\U\cdot\nnu - \int_\gamma\jmp{\tt[\dt\uu,\dt\U]}\cdot\dt\U = \int_\gamma\dt\FF\cdot\dt\U. }
...
\ml{ \label{eq:energy_sum} \int_{\Omega_m}(\CC_m\nabla\dt\uu:\nabla\dt\uu ) + \int_\gamma\jmp{\tt[\dt\uu,\dt\U]\cdot\dt(\uu-\U)}\\
- \int_\gamma\alpha_f\jmp{\dt p \dt\uu}\cdot\nnu
...
\mls{ \int_{\Omega_m}\left(\CC_m\nabla\uu:\nabla\vv - \alpha_m p\div\vv\right)\\
 + \delta\int_\gamma\avg{\CC_f\agrad[\uu,\U]:\agrad[\vv,\V]-\alpha_f P \adiv[\vv,\V]}
  = \dual{(\ff,\FF)}{(\vv,\V)}_\Vel, }
which is (i) from \eqref{eq:biot_weak}.
...
+ \delta \int_\gamma \avg{\tn K_f\agrad[p,P]\cdot\agrad[q,Q]}\\
= \dual{(g,G)}{(q,Q)}_\Hf, }
which is (ii) from \eqref{eq:biot_weak}.
