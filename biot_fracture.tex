...
\eq{ \label{eq:interface_el} (\tn C_m(\nabla\uu_{|\Omega_m})^\opm)\nnu = \tt^\opm[\uu,\ubar] \mbox{ in }\gamma. }
...
\begin{subequations}
\begin{itemize}
\item[(i)] the Biot equations in the matrix:
\begin{align}
-\div(\tn C_m\nabla\uu) + \alpha_m\nabla p &= \ff &&\mbox{ in }(0,T)\times\Omega_m,\\
\dt\left(S_m p + \alpha_m\div\uu\right) - \div(\tn K_m\nabla p) &= g &&\mbox{ in }(0,T)\times\Omega_m,
\end{align}

\item[(ii)] the reduced Biot equations in the fracture:
\ml{ -\delta\div_\tau\left(\tn C_f\avg{\agrad[\uu,\U]}\right) + \delta\alpha_f\nabla_\tau P - \jmp{\tt[\uu,\U]}\\
+ \alpha_f\jmp{p}\nnu = \FF \mbox{ in }(0,T)\times\gamma, }
\ml{ \delta\prtl_t\left(S_f P + \alpha_f\div_\tau\U\right) -\delta\div_\tau(\tn K_f\nabla_\tau P)\\
+ \alpha_f\jmp{\prtl_t\uu}\cdot\nnu - \jmp \pphi[p,P] = G \mbox{ in }(0,T)\times\gamma, }

\item[(ii)] the interface conditions:
\eq{ (\tn C_m(\nabla\uu)^\opm)\nnu = \tt^\opm[\uu,\U], \quad \tn K_m(\nabla p)^\opm\cdot\nnu = \pphi^\opm[p,P] \quad \mbox{ in }(0,T)\times\gamma, }
...
\item[(iv)] the initial conditions:
\begin{align}
