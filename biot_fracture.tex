...
\def\uu{\vc u}
\def\V{\vc V}
\def\Vel{{\mathcal H}} % Sobolev space for elasticity
...
\eq{ (\nabla_\nu\uu)^\opm \approx \pm\frac2\delta(\uu^\opm-\ubar)\otimes\nnu =: \Delta_\nu^\opm = \Delta_\nu^\opm[\uu,\ubar]. }
% In this secion, we shall write $\Delta_\nu^\opm$ in place of $\Delta_\nu^\opm[\uu^\opm,\ubar]$ to simplify notation.
% 
...
\eq{ \label{eq:lin_el_frac} -\delta\div_\tau\tn C_f\left(\nabla_\tau\ubar+\avg{\Delta_\nu}\right) + \delta\nabla_\tau(\alpha_f\pbar) - \jmp{\vc T} + \alpha_f\jmp{p}\nnu = \delta\overline\ff. }
...
where the approximate normal fluxes $F^\opm$ are given by
\eq{ \label{eq:def_F} F^\opm = F^\opm[p,\pbar] := \pm k_f\frac2\delta(p^\opm-\pbar). }
% 
...
\end{align}
\end{subequations}
Here and in what follows, $\Delta_\nu^\opm=\Delta_\nu^\opm[\uu,\U]$ and $\vc T^\opm=\vc T^\opm[\uu,\U]$.
Let us define the space 
\eq{ \Vel :=H^1_0(\Omega_m;\Real^d)\times H^1_0(\gamma;\Real^d) }
...
\eq{\label{eq:korn_conv_approx_grad} \ep_\tau\V_n+\ep_\nu^\opm[\vv_n,\V_n] \to 0 \mbox{ strongly in }L^2(\gamma;\Real^{d\times d}),~n\to\infty. }
...
With help of the compact embeddings $H^1(\Omega_m)\hookrightarrow L^2(\gamma)$ and $H^1(\gamma)\hookrightarrow L^2(\gamma)$ we deduce from \eqref{eq:korn_conv_approx_grad} that $\ep_\tau\V_n\to 0$ strongly in $L^2(\gamma;\Real^{d\times d})$, $n\to\infty$.
...
\begin{lemma}
The bilinear form $a$ is elliptic in $\Vel$:
