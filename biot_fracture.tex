...
We shall use the calculus from section \ref{sec:calculus} to derive appropriate equations that are satisfied by the averages $(\ubar,\pbar)$ in $(0,T)\times\gamma$.
...
We get:
\eq{ \label{eq:darcy_int} \prtl_t\left(\delta S_f\pbar + \delta\alpha_f\overline{\div\uu}\right) + \delta\overline{\div\qq} = \delta\overline g \mbox{ in }(0,T)\times\gamma. }
...
We want to find functions $\uu,p$ defined in $(0,T)\times\Omega_m$ and $\U,P$ defined in $(0,T)\times\gamma$, which satisfy:
...
\eq{ \label{eq:mixed_dim_problem_el_frac_perm} \tag{\ref{eq:mixed_dim_problem_el_frac}'} -\delta\div_\tau\left(\CC_f\avg{\agrad[\uu,\U]}\right) + \delta\alpha_f\grad_\tau P - \jmp{\tt[\uu,\U]} = \FF \mbox{ in }(0,T)\times\gamma, }
...
In this section we consider only the mechanical part of the problem \eqref{eq:mixed_dim_perm}.
...
To ensure the ellipticity of the bilinear form $a$, we first prove a suitable inequality for functions in $\Vel$.
...
In particular, $(p,P):=\mathcal F_{\vc\beta}(g,G)$ satisfies the initial conditions \eqref{eq:init_p}--\eqref{eq:init_P} and the integral identity
...
\subsection{Auxiliary inequalities and identities}
...
\subsection{Energy equality for \eqref{eq:mixed_dim_problem}}\label{sec:app_energy_eq}
...
\subsection{Weak formulation of \eqref{eq:mixed_dim_perm}}\label{sec:app_weak_form}

In what follows we derive the integral identities (ii)--(iii) from \eqref{eq:biot_weak}.
Multiplying the equations \eqref{eq:mixed_dim_problem_el_mat}, \eqref{eq:mixed_dim_problem_el_frac_perm} by test functions $(\vv,\V)\in\Vel$, integrating them by parts, adding and applying the boundary conditions \eqref{eq:mixed_dim_problem_bc_mat}--\eqref{eq:mixed_dim_problem_bc_frac}, we arrive at the identity
