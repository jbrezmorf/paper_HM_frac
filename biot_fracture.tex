...
\title{Mixed-dimensional models of poroelasticity}
\author{Jan Březina}
...
\js{
There are two main strategies of solving poromechanics systems: monolithic (see e.g. \cite{showalter2000diffusion,zenisek1984existence}) and iterative methods.
...
We recall that the components of the Jacobian matrix and of the tensor divergence are given by $\nabla\vv=\left[\tfrac{\prtl v_i}{\prtl x_j}\right]_{i,j=1}^d$ and \js{$\div\tn A = \left[\sum_{j=1}^d\tfrac{\prtl a_{ij}}{\prtl x_j}\right]_{i=1}^d$}, respectively.
...
\begin{lemma}
The bilinear form $a$ is bounded and elliptic and the linear form $l$ is bounded in $\Vel$.
\js{
...
\js{
By differentiating \eqref{eq:el_weak} with respect to time we observe that the time derivatives of $(\uu,\U)$ satisfy for all $(\vv,\V)\in\Vel$:
...
It is clear that $\dt l$ satisfies:
\eqs{ \dt l((\vv,\V)) \le \left(\norm{(\dt\ff,\dt\FF)}_{\Vel^*} + C_4\norm{(\dt p,\dt P)}_\Hf\right)\norm{(\vv,\V)}_\Vel. }
}
Since the solution mappings $(\ff,\FF,p,P) \mapsto (\uu, \U)$
\js{
and $(\dt\ff,\dt\FF,\dt p,\dt P)\mapsto(\dt\uu,\dt\U)$
}
are linear and bounded, we obtain:
\begin{corollary}
...
\begin{subequations}
    ...
    \begin{align}
      ...
      + \delta\int_\gamma\avg{\tn K_f\agrad[p,P]\cdot\agrad[q,Q]}
      = \dual{\js{(\tilde g,\tilde G)}}{(q,Q)}_\Hf.
    \end{align}
\end{subequations}}
\js{
...
In particular, $(p,P):=\mathcal F_{\vc\beta}(\tilde g,\tilde G)$ satisfies the initial conditions 
...
\eqs{ (p^{n+1},P^{n+1}) := \mathcal F_{\vc\beta}(\tilde g^n,\tilde G^n), }
\js{
where
...
\js{
Then the following expression $\rho_{\vc\beta}$ is a distance in $\mathcal X$:
}
...
\js{
Further, $(\mathcal X,\rho_{\vc\beta})$ is a complete metric space.
}
We shall prove that $\mathcal M_{\vc\beta}$ is a contraction with respect to $\rho_{\vc\beta}$.
...
\end{align*}
We subtract the equations \js{\eqref{eq:el_dt} for $(\dt\uu^{n+1},\dt\U^{n+1})$ and $(\dt\uu^n,\dt\U^n)$} and test by $(\ee_\uu^{n+1},\ee_\U^{n+1})$.
We obtain:
...
Next we subtract the equations \eqref{eq:flow_problem_beta} for $(p^{n+1},P^{n+1})$ and $(p^n,P^n)$, test by $(e_p^{n+1},e_P^{n+1})$, integrate in time and use \eqref{eq:pos_def_K}, which yields for a.a. $t\in\I$:
\ml{ 
...
The numerical solution based on the proposed splitting will be a subject of a following paper.
The problem can be further extended to the case of embedded fracture with variable aperture and intersecting fractures, further one should consider contact and friction conditions \js{(such as in \cite{berge2019finite})}, to name a few challenging tasks.
