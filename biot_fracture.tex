...
\end{lemma}
\begin{proof}
Let us assume for contradiction that there is a sequence $\{(\vv_n,\V_n)\}\subset V$ such that
...
Then there is an element $(\vv,\V)\in V$ such that
\eq{\label{eq:korn_weak_vV} (\vv_n,\V_n)\weakly (\vv,\V) \mbox{ weakly in }V. }
From \eqref{eq:korn_asm_contra} and the Korn inequality in $H^1_0(\Omega_m)$ it follows that
...
The Korn inequality in $H^1_0(\gamma)$ then implies that $\V_n\to 0$ strongly in $H^1_0(\gamma;\Real^d)$.
